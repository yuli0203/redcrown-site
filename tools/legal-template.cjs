...
<main id="main" class="rc-policy-main"><p class="rc-eyebrow">POLICIES AND SUPPORT</p><h1>${data.name}</h1><p>${data.scope}</p><p class="rc-updated">Last updated: 21 September 2026</p><div class="rc-policy-toc" role="navigation" aria-label="Sections">${sections.map(([id,label])=>`<a href="#${id}">${label}</a>`).join('')}</div>${sections.map(([id,label])=>`<section id="${id}"><h2>${label}</h2>${data[id]}</section>`).join('')}</main>
