...
function page(data) {
  return `<!doctype html>
<html lang="en"><head><script src="/shared/language.js"></script><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${data.name} | Policies and support</title><meta name="description" content="${data.name} privacy policy, terms of use, accessibility statement and support."><link rel="canonical" href="https://redcrowninteractive.com/${data.path.replace('index.html','')}"><link rel="icon" href="/favicon.ico"><link rel="stylesheet" href="/shared/legal.css"></head>
...
<main id="main" class="rc-policy-main"><p class="rc-eyebrow">POLICIES AND SUPPORT</p><h1>${data.name}</h1><p>${data.scope}</p><p class="rc-updated">Last updated: ${data.updated || '21 September 2026'}</p><div class="rc-policy-toc" role="navigation" aria-label="Sections">${sections.map(([id,label])=>`<a href="#${id}">${label}</a>`).join('')}</div>${sections.map(([id,label])=>`<section id="${id}"><h2>${label}</h2>${data[id]}</section>`).join('')}</main>
