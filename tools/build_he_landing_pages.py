...
<title>{title} | Red Crown Interactive</title><meta name="description" content="{description}"><link rel="canonical" href="https://redcrowninteractive.com/he/{slug}/"><link rel="icon" href="/favicon.ico"><link rel="stylesheet" href="/site.css"><link rel="stylesheet" href="/he/solutions.css?v=20260913-1">
...
<script type="application/ld+json">{{"@context":"https://schema.org","@type":"Service","name":"{title}","provider":{{"@type":"Organization","name":"Red Crown Interactive","url":"https://redcrowninteractive.com/"}},"areaServed":"IL","url":"https://redcrowninteractive.com/he/{slug}/"}}</script>{faq_schema}</head><body class="campaign-page">
...
for p in PAGES:
    ...
    description = p.get('description', p['lead'])[:155]
    wa = escape(f"שלום, אני מתעניין/ת ב{p['title']}").replace(' ', '%20')
    context = {k: v for k, v in p.items() if k not in {'uses', 'bullets', 'description'}}
