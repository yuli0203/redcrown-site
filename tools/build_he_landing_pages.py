...
<title>{title} | Red Crown Interactive</title><meta name="description" content="{description}"><link rel="canonical" href="https://redcrowninteractive.com/he/{slug}/"><link rel="icon" href="/favicon.ico"><link rel="stylesheet" href="/site.css"><link rel="stylesheet" href="/he/solutions.css?v=20260910-status">
...
<main><section class="hero-shell"><div class="hero-grid"></div><div class="hero-glow"></div><div class="wrap hero"><div class="hero-copy"><div class="eyebrow">{eyebrow}</div><h1>{title}</h1><p>{lead}</p><ul class="hero-benefits">{bullets}</ul><div class="hero-actions"><a class="btn" href="#contact">לשיחת היתכנות קצרה</a><span class="quiet">ללא התחייבות · אפיון, עיצוב ופיתוח תחת אחריות אחת</span></div></div><div class="hero-stage">{hero_visual}</div></div><div class="scroll-cue">גלו עוד <i></i></div></section>
<div class="trust"><div class="wrap"><span>ניסיון המייסדת ופרויקטים מקצועיים</span><div class="trust-logos"><img src="/assets/Companies/technion_w.png" alt="הטכניון"><img src="/assets/Companies/philips_w.png" alt="Philips"><img src="/assets/Companies/playtika_w.png" alt="Playtika"><img src="/assets/Companies/tel_aviv_university_w.png" alt="אוניברסיטת תל אביב"><img src="/assets/Companies/israelnavy_w.png" alt="חיל הים"></div></div></div>
<section class="wrap" id="solutions"><h2 class="section-title">{solutions_title}</h2><p class="section-lead">{solutions_lead}</p>{ar_service_note}<div class="cards">{uses}</div></section>
...
for p in PAGES:
    ...
    if p['slug'] == 'vr-development':
        context['ar_service_note'] = '<div class="xr-service-detail"><article><h3>פיתוח VR: סביבה שנכנסים לתוכה</h3><p>סימולציות, הדרכה וניסויים במציאות מדומה, עם אינטראקציה, כלי מדריך ואיסוף נתונים. מהתוכן והתלת־ממד ועד הפעלה בחומרת היעד.</p><a href="/work/fugacity-vr-lab/">לפרויקט מעבדת ה־VR בטכניון ←</a></article><article><h3>פיתוח AR: מידע שפוגש את העולם האמיתי</h3><p>המחשת מוצרים ותהליכים במציאות רבודה, על מכשירים ניידים או משקפי מציאות משולבת. שילוב מודלים תלת־ממדיים, מעקב ידיים וממשקים מרחביים.</p><a href="/work/enzymatic-lab-ar/">לפרויקט המעבדה במציאות רבודה ←</a></article></div>'
    ...
    context['contact_lead'] = "גם רעיון ראשוני מספיק. נבחן יחד את הצורך, המשתמשים וההיתכנות, ונגדיר מה נכון לבדוק לפני שמתחילים בפיתוח."
