...
- **Cloudflare Workers Paid, $5/month.** Required: building and signing the
  receipt PDFs takes about 0.3 s of CPU per payment; the free plan allows 10 ms.
...
1. **Cloudflare:** upgrade to Workers Paid. Create the KV namespace and paste
...
9. **Certificate** (when it arrives), then send the registered letter:
   ```
   base64 -w0 your-certificate.p12 > cert.b64
   npx wrangler kv key put --binding PAYMENTS --remote "signing:p12" --path cert.b64
   npx wrangler secret put SIGNING_P12_PASSWORD
   rm cert.b64
   ```
...
- `workers/pay/src/sign.js`: digital signature
...
- `workers/pay/assets/`: IBM Plex Sans Hebrew (SIL OFL) and the logo, embedded
  by `tools/embed-assets.mjs`
...
node tools/embed-assets.mjs                  # after changing assets/
