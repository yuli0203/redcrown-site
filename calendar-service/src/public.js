...
function publicData(workspace){return {pageName:workspace.pageName,hostName:workspace.hostName||workspace.pageName,calendarDisplay:workspace.calendarDisplay,timezone:workspace.timezone,slug:workspace.slug,accent:workspace.accent,background:workspace.background,text:workspace.text,logo:workspace.logo,photo:workspace.photo,meetings:workspace.meetings.filter(m=>m.enabled).map(({id,title,description,duration,location,notice,horizon})=>({id,title,description,duration,location,notice,horizon}))};}
async function available(host,meeting,range,env,provider,previousId=null){
 ...
 const padding=8*3600000;let external;try{external=await provider.readAvailability(linked,range.start-padding,range.end+padding,env);}catch(error){console.warn('Availability provider failure',error.name,error.providerStatus||'');throw new Problem('The host calendar could not be checked. Please try again later or contact the host.',503);}
...
export async function publicRoutes(request,env,provider,path){
 ...
 if(path.startsWith('/public/')){
  ...
  if(parts[3]==='book'&&method==='POST'){
   ...
   assert(input.meetingName===undefined||(typeof input.meetingName==='string'&&input.meetingName.length<=160&&!/[\r\n\x00-\x1f]/.test(input.meetingName)),'Enter a meeting name of up to 160 characters.');
   const eventTitle=input.meetingName?.trim()||`${input.name.trim()} and ${host.workspace.hostName||host.workspace.pageName}`;
   ...
   if(booking){const prior=JSON.parse(booking.data);assert(booking.start===input.start&&booking.meeting_id===input.meetingId&&prior.email===input.email&&(!Object.hasOwn(prior,'meetingName')||prior.meetingName===(input.meetingName?.trim()||''))&&JSON.stringify([...(prior.participants||[])].sort())===JSON.stringify(participants),'This booking request was already used.',409);}
   else {
    ...
    const data=JSON.stringify({hostName:host.workspace.pageName,hostEmail:connection.email,inviteeTimezone:input.inviteeTimezone||null,title:eventTitle,meetingName:input.meetingName?.trim()||'',location:meeting.location,reminderMinutes:meeting.reminderMinutes||0,reminderEmail:meeting.reminderEmail||'',name:input.name.trim(),email:input.email,participants,notes:input.notes||'',timezone:host.workspace.timezone,manageUrl:`${env.PUBLIC_ORIGIN}/calendar/meet/?booking=${encodeURIComponent(id)}#${manage}`});
   ...
   return json({id:booking.id,start:booking.start,end:booking.end,status:booking.status,title:JSON.parse(booking.data).title,manageToken:await managementToken(booking.id,env)},201);
  }
 }
