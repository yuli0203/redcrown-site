...
async function available(host,meeting,range,env,provider){
 ...
 return slots(host.workspace,meeting,busy,range,Date.now(),reserved.filter(b=>b.meeting_id===meeting.id));
}
...
export async function publicRoutes(request,env,provider,path){
 ...
 if(path.startsWith('/public/')){
  ...
  if(parts[3]==='slots'&&method==='GET'){
   ...
   return json({slots:await available(host,meeting,range,env,provider),timezone:host.workspace.timezone});
  }
  if(parts[3]==='book'&&method==='POST'){
   ...
   if(booking){const prior=JSON.parse(booking.data);assert(booking.start===input.start&&booking.meeting_id===input.meetingId&&prior.email===input.email,'This booking request was already used.',409);}
    ...
    const date=localDate(input.start,host.workspace.timezone),range=dateRange(date,1,host.workspace.timezone),choices=await available(host,meeting,range,env,provider);
    assert(choices.some(s=>s.start===input.start),'This time is no longer available. Choose another.',409);
    let previous=null;
    if(input.previousId){previous=await one(db,'SELECT * FROM bookings WHERE id=? AND uid=?',input.previousId,host.uid);assert(previous&&previous.status==='confirmed'&&previous.meeting_id===meeting.id&&await hash(input.rescheduleToken||'')===previous.manage_hash,'This booking cannot be rescheduled. Refresh its management page.',409);}
 ...
 if(path.startsWith('/booking/')){
  ...
  if(path.endsWith('/cancel')&&method==='POST'){
   ...
   await run(db,"UPDATE bookings SET status='cancelling' WHERE id=? AND status='confirmed'",id);
