...
export function providerConfig(provider,env){
 ...
 return {clientId,secret,authorize:microsoft?'https://login.microsoftonline.com/common/oauth2/v2.0/authorize':'https://accounts.google.com/o/oauth2/v2/auth',token:microsoft?'https://login.microsoftonline.com/common/oauth2/v2.0/token':'https://oauth2.googleapis.com/token',scope:microsoft?'openid email offline_access User.Read Calendars.ReadWrite':'openid email https://www.googleapis.com/auth/calendar.calendarlist.readonly https://www.googleapis.com/auth/calendar.events https://www.googleapis.com/auth/calendar.events.freebusy',redirect:`${env.PUBLIC_ORIGIN}/calendar/api/oauth/${provider}/callback`};
}
...
export async function readAvailability(connections,start,end,env,{details=false,timezone='UTC'}={}){
 ...
 for(const connection of connections){const selected=JSON.parse(connection.calendars).filter(c=>c.selected);if(!selected.length)continue;const token=await tokenFor(connection,env);
  ...
  for(const calendar of selected){let url=connection.provider==='google'?`${google}/calendars/${encodeURIComponent(calendar.id)}/events?${new URLSearchParams({timeMin:new Date(start).toISOString(),timeMax:new Date(end).toISOString(),singleEvents:'true',maxResults:'250',fields:'nextPageToken,items(id,summary,start,end,status,transparency)'})}`:`${graph}/me/calendars/${encodeURIComponent(calendar.id)}/calendarView?${new URLSearchParams({startDateTime:new Date(start).toISOString(),endDateTime:new Date(end).toISOString(),'$top':'250','$select':'id,subject,start,end,isAllDay,isCancelled,showAs'})}`;
   const base=url;try{for(let pageNumber=0;url&&pageNumber<40;pageNumber++){
    const page=await request(url,token,{headers:{Prefer:'outlook.timezone="UTC"'}});
    for(const event of page.items||page.value||[]){if(event.status==='cancelled'||event.isCancelled)continue;const from=eventTime(event.start,timezone),to=eventTime(event.end,timezone);assert(Number.isFinite(from)&&Number.isFinite(to)&&from<to,'Calendar returned an invalid event time.',503);const blocking=connection.provider==='google'?event.transparency!=='transparent':!['free','workingElsewhere'].includes(event.showAs);if(connection.provider==='microsoft'&&blocking)busy.push({start:from,end:to});if(details)events.push({start:from,end:to,title:event.summary||event.subject||'Busy',calendar:calendar.name,allDay:Boolean(event.start?.date||event.isAllDay),busy:blocking});}
...
export async function writeBooking(connection,calendarId,booking,env){const token=await tokenFor(connection,env),data=JSON.parse(booking.data),googleEventId=booking.id.replaceAll('-','');
 const content=connection.provider==='google'?{id:googleEventId,summary:data.title,description:`Booked through Red Crown Calendar.\n${data.notes||''}`,location:data.location||'',start:{dateTime:new Date(booking.start).toISOString()},end:{dateTime:new Date(booking.end).toISOString()},attendees:[{email:data.email,displayName:data.name}]}:{transactionId:booking.id,subject:data.title,body:{contentType:'text',content:`Booked through Red Crown Calendar.\n${data.notes||''}`},location:{displayName:data.location||''},start:{dateTime:new Date(booking.start).toISOString(),timeZone:'UTC'},end:{dateTime:new Date(booking.end).toISOString(),timeZone:'UTC'},attendees:[{emailAddress:{address:data.email,name:data.name},type:'required'}]};
