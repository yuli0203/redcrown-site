...
async function request(url,token,options={}){const response=await fetch(url,{...options,headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json',...(options.headers||{})},redirect:'manual',signal:AbortSignal.timeout(15000)});if(!response.ok)throw Object.assign(new Problem(response.status===401||response.status===403?'Calendar access needs to be reconnected.':'The calendar provider could not complete this request. Please retry.',503),{providerStatus:response.status});return response.status===204?{}:response.json();}
...
export async function writeBooking(connection,calendarId,booking,env){const token=await tokenFor(connection,env),data=JSON.parse(booking.data),googleEventId=booking.id.replaceAll('-','');
 const content=connection.provider==='google'?{id:googleEventId,transparency:'opaque',summary:data.title,description:`Booked through Red Crown Calendar.\n${data.notes||''}${data.manageUrl?'\n\nManage or cancel this booking: '+data.manageUrl:''}`,location:data.location||'',start:{dateTime:new Date(booking.start).toISOString()},end:{dateTime:new Date(booking.end).toISOString()},attendees:[{email:data.email,displayName:data.name},...(data.participants||[]).map(email=>({email}))]}:{transactionId:booking.id,showAs:'busy',subject:data.title,body:{contentType:'text',content:`Booked through Red Crown Calendar.\n${data.notes||''}${data.manageUrl?'\n\nManage or cancel this booking: '+data.manageUrl:''}`},location:{displayName:data.location||''},start:{dateTime:new Date(booking.start).toISOString(),timeZone:'UTC'},end:{dateTime:new Date(booking.end).toISOString(),timeZone:'UTC'},attendees:[{emailAddress:{address:data.email,name:data.name},type:'required'},...(data.participants||[]).map(email=>({emailAddress:{address:email},type:'required'}))]};
 ...
 if(connection.provider==='google'){try{const existing=await request(`${google}/calendars/${encodeURIComponent(calendarId)}/events/${googleEventId}`,token);assert(existing.status!=='cancelled','The calendar event was cancelled. Contact the host before retrying.',409);return existing;}catch(error){if(error.providerStatus!==404)throw error;}}
