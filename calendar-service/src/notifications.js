...
export async function sendBookingNotifications(env,now=Date.now(),send=fetch,bookingId=null){
 ...
 const rows=(await env.DB.prepare(`SELECT * FROM bookings WHERE status='confirmed' AND notification_sent_at IS NULL AND start>? AND created_at>? AND json_extract(data,'$.hostEmail') IS NOT NULL AND (notification_attempt_at IS NULL OR notification_attempt_at>?) AND (? IS NULL OR id=?) ORDER BY created_at LIMIT 50`).bind(now,now-23*3600000,now-23*3600000,bookingId,bookingId).all()).results;
