...
export function validateWorkspace(input){
 ...
 const meetings=(input.meetings||[]).map(m=>({id:text(m.id,80,true),title:text(m.title,80,true),description:text(m.description||'',500),duration:integer(m.duration,1,480),before:integer(m.before??0,0,240),after:integer(m.after??0,0,240),notice:integer(m.notice??120,0,43200),horizon:integer(m.horizon??60,1,365),interval:integer(m.interval??15,1,240),dailyLimit:integer(m.dailyLimit??0,0,100),location:text(m.location||'',200),enabled:m.enabled!==false}));
 ...
 const result={pageName:text(input.pageName||'',60),slug,timezone:zone,weekly:normalized,exceptions,meetings,published:input.published===true,destination:input.destination||null};
