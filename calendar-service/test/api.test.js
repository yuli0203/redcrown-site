...
async function fixture(){const DB=database(),env={DB,PUBLIC_ORIGIN:'http://localhost',TOKEN_ENCRYPTION_KEY:Buffer.alloc(32,7).toString('base64')};let failed=false,writes=0;
 ...
 const request=()=>({meetingId:'intro',start,name:'Guest',email:'guest@example.test',requestId:crypto.randomUUID()});return {DB,env,api,workspace,start,request,fail(value){failed=value;},writes:()=>writes};}
