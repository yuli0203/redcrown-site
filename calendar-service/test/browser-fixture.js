...
const fixturePort=Number(process.env.FIXTURE_PORT||8771);
const root=resolve(fileURLToPath(new URL('../../',import.meta.url))),DB=database(),env={DB,PUBLIC_ORIGIN:`http://127.0.0.1:${fixturePort}`,TOKEN_ENCRYPTION_KEY:Buffer.alloc(32,5).toString('base64'),GOOGLE_CLIENT_SECRET:'fixture-only',MICROSOFT_CLIENT_SECRET:'fixture-only'};
...
if(!process.argv.includes('--new-user'))await DB.prepare('INSERT INTO profiles(uid,slug,data,version,updated_at) VALUES(?,?,?,?,?)').bind('fixture','sample-host',JSON.stringify(workspace),1,Date.now()).run();
...
if(url.pathname==='/calendar/auth.js'){res.writeHead(200,{'Content-Type':'text/javascript'});res.end(`window.CrownAuth={token:async()=> 'fixture'};window.addEventListener('load',async()=>{await CrownAPI.ready;document.querySelector('#auth-identity').textContent='Sample account - test only';document.querySelector('#auth-account').hidden=false;if(document.querySelector('#auth-open'))document.querySelector('#auth-open').hidden=true;document.dispatchEvent(new CustomEvent('crown-auth-change',{detail:{uid:'fixture',displayName:'Sample host'}}));});`);return;}
const decoded=decodeURIComponent(url.pathname);if(decoded.split('/').some(p=>p.startsWith('.')||['calendar-service','tools'].includes(p)))throw Error();let path=resolve(root,'.'+decoded);if(!path.startsWith(root+sep))throw Error();if((await stat(path)).isDirectory())path=resolve(path,'index.html');res.writeHead(200,{'Content-Type':types[extname(path)]||'application/octet-stream','Cache-Control':'no-store'});res.end(await readFile(path));}catch{res.writeHead(404);res.end('Not found');}}).listen(fixturePort,'127.0.0.1',()=>console.log(`Isolated sample-only QA server: http://127.0.0.1:${fixturePort}/calendar/`));
