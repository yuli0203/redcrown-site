...
test('Google all-day event names use the host time zone and private details can fail safely',async t=>{
 const env={DB:database(),GOOGLE_CLIENT_ID:'client',GOOGLE_CLIENT_SECRET:'secret',TOKEN_ENCRYPTION_KEY:Buffer.alloc(32,3).toString('base64'),PUBLIC_ORIGIN:'http://localhost'},connection={id:'c',uid:'u',provider:'google',refresh_token:await encrypt('refresh',env),calendars:JSON.stringify([{id:'work',name:'Work',selected:true}])};
 let detailsFail=false;
 t.mock.method(globalThis,'fetch',async url=>{url=String(url);if(url.includes('oauth2'))return response({access_token:'access'});if(url.endsWith('freeBusy'))return response({calendars:{work:{busy:[{start:'2026-09-20T21:00:00Z',end:'2026-09-21T21:00:00Z'}]}}});return detailsFail?response({},403):response({items:[{summary:'Holiday',start:{date:'2026-09-21'},end:{date:'2026-09-22'}}]});});
 const data=await readAvailability([connection],Date.parse('2026-09-20'),Date.parse('2026-09-23'),env,{details:true,timezone:'Asia/Jerusalem'});assert.equal(data.events[0].start,Date.parse('2026-09-20T21:00:00Z'));assert.equal(data.events[0].title,'Holiday');
