...
## Branded host booking notifications

Apply migration 0005 before deploying. New bookings snapshot the connected destination account email, host name and invitee time zone. The confirmation sends a separate Red Crown Calendar HTML/plain-text notification to that host email after calendar confirmation. Google/Microsoft control their own invitation layout. No provider invitation is replaced.

Configure RESEND_API_KEY and a verified NOTIFICATION_FROM sender (REMINDER_FROM is the fallback). Without them, no branded email is sent. Failed attempts retry through the one-minute Worker cron, with a stable provider idempotency key and a 23-hour cutoff. Cancelled, unconfirmed and past events are excluded. Local development attempts delivery after confirmation only when configured; it has no cron. Existing bookings without a host-email snapshot are not backfilled. Do not enable live mail until the sender domain is verified and a controlled mailbox test passes. Recipient addresses and management links stay out of the public host response.
