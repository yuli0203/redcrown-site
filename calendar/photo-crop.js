(() => {
  ...
  window.CrownPhotoCrop={create(onChange){
    ...
    function reset(){zoom.value='1';x.value=y.value='50';draw();}
    for(const input of [zoom,x,y])input.addEventListener('input',draw);
    $('profile-crop-reset').addEventListener('click',reset);
    canvas.addEventListener('pointerdown',event=>{if(!source)return;canvas.setPointerCapture(event.pointerId);drag={left:event.clientX,top:event.clientY,x:Number(x.value),y:Number(y.value)};});
