(async()=>{
 const response=await fetch('/calendar/he/strings.json?v=auth-he-1');if(!response.ok)return;const dict=await response.json();
 const selectors=['#auth-link-microsoft','#auth-account','.entry-card','.footer-signin','#auth-message','#workspace-auth-status','#feedback-status','#calendar-tip-note','#calendar-feedback-form button','#theme-toggle'];
