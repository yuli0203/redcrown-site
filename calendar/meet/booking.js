(async()=>{
 ...
 try{
  ...
  for(const meeting of meetings){
   const card=make('section','','public-meeting'),summary=make('div','','meeting-summary');summary.append(make('h2',meeting.title),make('p',`${meeting.duration} minutes${meeting.location?' - '+meeting.location:''}`),make('p',meeting.description));
   card.append(summary);
   const link=make('a','View available times','booking-button');link.href=`/calendar/meet/?user=${encodeURIComponent(slug)}&meeting=${encodeURIComponent(meeting.id)}`;
   if(!query.get('meeting'))summary.append(link);$('#public-meetings').append(card);
   if(query.get('meeting')){document.body.classList.add('booking-detail');const back=make('a','All meeting types','meeting-back');back.href=`/calendar/meet/?user=${encodeURIComponent(slug)}`;summary.prepend(back);await showScheduler(card,meeting,slug,zone);}
  }
 ...
 async function showScheduler(card,meeting,slug,initialZone){
  ...
  const today=new Date(),firstMonth=new Date(today.getFullYear(),today.getMonth(),1);let month=new Date(firstMonth),selectedDate='',currentSlots=[];
  ...
  const previous=button('Previous month',()=>{month.setMonth(month.getMonth()-1);load();}),next=button('Next month',()=>{month.setMonth(month.getMonth()+1);load();}),nav=make('div','','booking-month-nav');nav.append(previous,monthTitle,next);
  ...
  controls.append(label);panel.append(controls,status,layout,formArea);zone.addEventListener('change',()=>{selectedDate='';load();});await load();
  async function load(){
   const current=++revision;status.textContent='Checking availability...';grid.replaceChildren();days.replaceChildren();formArea.replaceChildren();monthTitle.textContent=month.toLocaleDateString(undefined,{month:'long',year:'numeric'});previous.disabled=month<=firstMonth;
   ...
   }catch(e){if(current===revision){status.textContent=e.message;grid.replaceChildren();}}
  }
  function bookingForm(slot){
   formArea.replaceChildren();const form=make('form','','guest-form'),heading=make('h3',new Date(slot.start).toLocaleString(undefined,{timeZone:zone.value,dateStyle:'full',timeStyle:'short'}));form.append(heading,make('p',`${meeting.duration} minutes - ${zone.value.replaceAll('_',' ')}${meeting.location?' - '+meeting.location:''}`,'booking-status'));
   const inputs={};for(const [key,title,type,max] of [['name','Your name','text',100],['email','Email address','email',254],['notes','Anything the host should know?','text',2000]]){const label=make('label',title),input=make(key==='notes'?'textarea':'input');if(key!=='notes')input.type=type;input.required=key!=='notes';input.maxLength=max;input.autocomplete=key==='notes'?'off':key;label.append(input);form.append(label);inputs[key]=input;if(originalGuest?.[key])input.value=originalGuest[key];}
   ...
   form.addEventListener('submit',async event=>{event.preventDefault();if(!form.reportValidity())return;submit.disabled=true;notice.textContent='Confirming with the calendar...';try{const result=await call(`/public/${encodeURIComponent(slug)}/book`,'POST',{meetingId:meeting.id,start:slot.start,requestId,name:inputs.name.value.trim(),email:inputs.email.value.trim(),notes:inputs.notes.value,turnstileToken:captchaToken,...(query.has('reschedule')?{previousId:query.get('reschedule'),rescheduleToken:location.hash.slice(1)}:{})});form.replaceChildren(make('h3',query.has('reschedule')?'Your meeting has been rescheduled':'Your meeting is confirmed'),make('p',new Date(result.start).toLocaleString(undefined,{timeZone:zone.value,dateStyle:'full',timeStyle:'short'})),make('p','The calendar invitation has been requested for your email. Keep the link below to manage your booking.'));const manage=make('a','Manage or cancel booking','booking-button');manage.href=`/calendar/meet/?booking=${encodeURIComponent(result.id)}#${result.manageToken}`;form.append(manage,button('Download calendar file',()=>downloadICS(meeting,result)));status.textContent='Booking confirmed.';layout.hidden=true;controls.hidden=true;}catch(e){notice.textContent=e.message;if(widget!==null){window.turnstile.reset(widget);captchaToken='';}if(e.status===409)requestId=crypto.randomUUID();}finally{submit.disabled=false;}});
   inputs.name.focus();
  }
 }
 async function manage(){const id=query.get('booking'),token=location.hash.slice(1);const result=await call(`/booking/${encodeURIComponent(id)}?token=${encodeURIComponent(token)}`);$('#host-name').textContent=result.title;$('#page-description').textContent=`${new Date(result.start).toLocaleString(undefined,{dateStyle:'full',timeStyle:'short'})} (${Intl.DateTimeFormat().resolvedOptions().timeZone}) - ${result.status}`;if(result.status==='confirmed'&&result.slug){const link=make('a','Choose a new time','booking-button');link.href=`/calendar/meet/?user=${encodeURIComponent(result.slug)}&meeting=${encodeURIComponent(result.meetingId)}&reschedule=${encodeURIComponent(id)}#${token}`;$('#public-meetings').append(link);}
