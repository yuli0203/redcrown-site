(async()=>{
 ...
 try{
  ...
  for(const meeting of meetings){
   ...
   if(query.get('meeting')){document.body.classList.add('booking-detail');step(2);const back=make('a','All meeting types','meeting-back');back.href=query.has('reschedule')?`/calendar/meet/?booking=${encodeURIComponent(query.get('reschedule'))}#${location.hash.slice(1)}`:`/calendar/meet/?user=${encodeURIComponent(slug)}`;if(query.has('reschedule'))back.textContent='Back to your booking';const identity=make('div','','booking-host-identity');
 ...
 async function showScheduler(card,meeting,slug,initialZone){
  ...
  function bookingForm(slot){
   ...
   form.addEventListener('submit',async event=>{event.preventDefault();inputs.name.setCustomValidity(inputs.name.value.trim()?'':'Enter your name.');if(!form.reportValidity())return;if(api.config.turnstileSiteKey&&!captchaToken){notice.textContent='Complete the security verification before booking.';return;}submit.disabled=true;notice.textContent='Confirming with the calendar...';try{pendingPayload??={meetingId:meeting.id,start:slot.start,requestId,name:inputs.name.value.trim(),email:inputs.email.value.trim(),notes:inputs.notes.value,...(query.has('reschedule')?{previousId:query.get('reschedule'),rescheduleToken:location.hash.slice(1)}:{})};for(const input of Object.values(inputs))input.readOnly=true;form.querySelector('button[type=button]').disabled=true;const result=await call(`/public/${encodeURIComponent(slug)}/book`,'POST',{...pendingPayload,turnstileToken:captchaToken});step(4);form.classList.add('booking-confirmed');form.replaceChildren(make('span','✓','confirmation-check'),make('h2',query.has('reschedule')?'Your meeting has been rescheduled':'Your meeting is confirmed'),make('p',new Date(result.start).toLocaleString(undefined,{timeZone:zone.value,dateStyle:'full',timeStyle:'short'})),make('p',meeting.title),make('p',`A calendar invitation has been requested for ${inputs.email.value.trim()}. Check your inbox and spam folder. You can also add the meeting to your calendar below.`),make('p',meeting.location||'Location details are provided by your host.'));const manage=make('a','Manage or cancel booking','booking-button');manage.href=`/calendar/meet/?booking=${encodeURIComponent(result.id)}#${result.manageToken}`;form.append(manage,calendarDownload(meeting,result),make('p','Works with Google Calendar, Outlook and Apple Calendar. If you accept the email invitation, you do not need to import it again.','booking-status'));status.textContent='Booking confirmed.';panel.querySelector('.booking-panel-title').textContent='You are booked';layout.hidden=true;controls.hidden=true;}catch(e){notice.textContent=e.message;if(widget!==null){window.turnstile.reset(widget);captchaToken='';}if(e.status&&e.status<500){pendingPayload=null;for(const input of Object.values(inputs))input.readOnly=false;form.querySelector('button[type=button]').disabled=false;if(e.status===409){requestId=crypto.randomUUID();notice.append(button('Choose another available time',()=>{formArea.replaceChildren();panelHeading.textContent='Select a date and time';load();}));}}else{notice.textContent+=' Your details are kept unchanged so retrying cannot create a different booking.';submit.textContent='Retry booking confirmation';}}finally{submit.disabled=false;}});
   inputs.name.focus();
  }
 }
 ...
 function calendarDownload(meeting,booking){const text=window.CrownCalendarFile.create(meeting,booking);const url=URL.createObjectURL(new Blob([text],{type:'text/calendar;charset=utf-8'})),link=make('a','Add to calendar (.ics)','booking-button');link.href=url;link.download='meeting.ics';window.addEventListener('pagehide',()=>URL.revokeObjectURL(url),{once:true});return link;}
