(async () => {
  ...
  notificationWarning.textContent=document.documentElement.lang.startsWith('he')?'הזמנות לפגישות נשלחות למשתתפים דרך ספק היומן. אימייל נפרד למארח אינו מופעל.':'Meeting invitations are sent to guests through the calendar provider. Separate host emails are not enabled.';
    ...
    try{const result=await window.CrownAPI.request('/bookings');if(version!==revision)return;const list=$('#upcoming-bookings-list');list.replaceChildren();
      for(const booking of result.bookings){const row=document.createElement('article');row.className='meeting-type-row';const text=document.createElement('div');const title=document.createElement('h3');title.textContent=booking.data.title;const meta=document.createElement('p');meta.className='sync-small';meta.textContent=`${new Date(booking.start).toLocaleString()} - ${booking.data.name} - ${booking.status}`;text.append(title,meta);row.append(text);
       if(booking.status==='confirmed'){
        ...
        const labels=he?{accepted:'האימייל הנפרד למארח התקבל לשליחה אצל ספק האימייל.',sending:'האימייל הנפרד למארח נשלח כעת.',retry:'האימייל הנפרד למארח ממתין לניסיון שליחה נוסף.',unknown:'לא ניתן לאמת את שליחת האימייל הנפרד למארח.',failed:'לא ניתן היה לשלוח את האימייל הנפרד למארח.',permission_required:'האימייל הנפרד למארח אינו זמין.',not_sent:'האימייל הנפרד למארח טרם נשלח.'}:{accepted:'Separate host email accepted by the email provider.',sending:'Sending separate host email.',retry:'Separate host email queued for retry.',unknown:'Separate host email delivery is uncertain.',failed:'Separate host email could not be sent.',permission_required:'Separate host email is unavailable.',not_sent:'Separate host email has not been sent.'};
        const delivery=document.createElement('p');delivery.className='sync-small';
        delivery.textContent=he?'הפגישה נשמרה. נשלחה לספק היומן בקשה להזמין את המשתתפים.':'Meeting saved. Guest invitations requested from the calendar provider.';
        if(window.CrownAPI.config?.bookingNotifications===true)delivery.textContent+=' '+(labels[state]||labels.not_sent);
