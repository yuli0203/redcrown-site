(() => {
 ...
 const zones=[...new Set(['UTC','Asia/Jerusalem',deviceZone,...Intl.supportedValuesOf('timeZone')])];
 const zoneSelect=$('#schedule-timezone'),zoneLabel=zoneSelect.parentElement;
 const searchLabel=make('label','Find a time zone'),search=make('input'),help=make('p'),shortcuts=make('div');
 search.type='search';search.id='timezone-search';search.placeholder='Search city or region, e.g. Jerusalem, London';search.autocomplete='off';searchLabel.append(search);
 help.className='sync-small settings-wide';help.setAttribute('role','status');help.id='timezone-help';zoneSelect.setAttribute('aria-describedby',help.id);
 shortcuts.className='settings-actions settings-wide';
 const aliases={'Asia/Jerusalem':'Israel / Jerusalem / Tel Aviv / ישראל / ירושלים','UTC':'Coordinated Universal Time / UTC'};
 function zoneName(zone){const offset=new Intl.DateTimeFormat('en',{timeZone:zone,timeZoneName:'shortOffset'}).formatToParts(new Date()).find(p=>p.type==='timeZoneName')?.value||'';return (zone==='Asia/Jerusalem'?'Israel / Jerusalem':zone.replaceAll('_',' '))+' ('+offset+')';}
 function renderZones(selected=zoneSelect.value||deviceZone){
  if(!zones.includes(selected))zones.push(selected);
  const query=search.value.trim().toLowerCase(),matches=zones.filter(z=>(zoneName(z)+' '+(aliases[z]||'')).toLowerCase().includes(query));zoneSelect.replaceChildren();
  const current=make('optgroup','');current.label='Selected time zone';const option=make('option',zoneName(selected));option.value=selected;current.append(option);zoneSelect.append(current);
  const results=make('optgroup');results.label=query?'Search results':'All time zones';for(const zone of matches.filter(z=>z!==selected)){const o=make('option',zoneName(zone));o.value=zone;results.append(o);}zoneSelect.append(results);zoneSelect.value=selected;
  help.textContent=query?(matches.length?matches.length+' matching time zones. Choose one from the list.':'No matches. Your selected time zone is unchanged.'):'Offsets shown are current and adjust automatically for daylight saving time.';
 }
 search.addEventListener('input',()=>renderZones());
 zoneSelect.addEventListener('change',()=>{search.value='';renderZones();});
 for(const [title,zone] of [['Use my device: '+deviceZone.replaceAll('_',' '),deviceZone],['Israel / Jerusalem','Asia/Jerusalem']]){const b=make('button',title);b.type='button';b.className='auth-button';b.addEventListener('click',()=>{search.value='';renderZones(zone);help.textContent=zoneName(zone)+' selected. Save availability to apply.';});shortcuts.append(b);}
 zoneLabel.before(searchLabel);zoneLabel.after(shortcuts,help);renderZones();
 ...
 function apply(data){search.value='';renderZones(data.timezone||deviceZone);$('#schedule-slug').value=data.slug||'';$('#weekly-hours').replaceChildren();$('#date-overrides').replaceChildren();days.forEach((day,index)=>{const row=make('div');row.className='weekly-day';row.dataset.day=index+1;const windows=make('div');windows.className='hours-windows';(data.weekly?.[index+1]??(index<5?[['09:00','17:00']]:[])).forEach(v=>windowRow(windows,v));const add=make('button','+ Hours');add.type='button';add.className='auth-link';add.addEventListener('click',()=>windowRow(windows));row.append(make('strong',day),windows,add);$('#weekly-hours').append(row);});for(const [date,values] of Object.entries(data.exceptions||{}))override(date,values);destination=data.destination||null;renderDestinations();}
